...
void ParticleFilter::InitByGuassian(const math::Vec3d &mean, const math::Mat3d &cov) {
  int i;
...
void ParticleFilter::ClusterStatistics(const SampleSetPtr &sample_set_ptr) {
  ...
  //work space
  math::Mat2d tmp_ws_mat;
  math::Vec4d tmp_ws_vec;
  size_t count;
  double weight;
...
SampleSetPtr ParticleFilter::GetCurrentSet() const {
...
void ParticleFilter::UpdateResample() {
  ...
  while (set_b->sample_count < this->max_samples_) {
    ...
    if (drand48() < w_diff)
      sample_b->pose = (random_pose_func_)(map_ptr_);
    else {
      double r;
      r = drand48();
...
void ParticleFilter::InitByModel(PfInitModelFunc init_fn, const std::shared_ptr<AmclMap> &init_data) {
  int i;
  auto set = this->sample_set_ptr_array_[current_set_];
  ...
  for (i = 0; i < set->sample_count; i++) {
    ...
    set->samples_vec[i].pose = init_fn(init_data);;
    // Add sample to histogram
