...
void ParticleFilterKDTree::InsertPose(math::Vec3d pose, double value) {
	math::Vec3d key;
	for (int i = 0; i < 3; i++) {
...
int ParticleFilterKDTree::GetCluster(math::Vec3d pose) {
	math::Vec3d key;
	ParticleFilterKDTreeNode *node_ptr;
...
ParticleFilterKDTreeNode *ParticleFilterKDTree::FindNode(ParticleFilterKDTreeNode *node_ptr, math::Vec3d key) {
	if (node_ptr->leaf) {
		if (IsEqualKey(key, node_ptr->key)) {
			return node_ptr;
		} else {
